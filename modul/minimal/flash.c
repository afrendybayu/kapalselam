...
static portTASK_FUNCTION( vLEDFlashTask, pvParameters )
{
	...
	unsigned portLONG ulCurrentState;
	...
	//xFlashRate = 333 + ( 333 * uxLED );
	xFlashRate = ledFLASH_RATE_BASE + ( ledFLASH_RATE_BASE * ( portTickType ) uxLED );
	xFlashRate /= portTICK_RATE_MS;

	// We will turn the LED on and off again in the delay period, 
	//so each delay is only half the total period.
