...
static volatile unsigned portBASE_TYPE uxRxLoops = comINITIAL_RX_COUNT_VALUE;		// 0 = comINITIAL_RX_COUNT_VALUE
...
portTickType xTimeToWait;
	...
	for( ;; )
	{
		...
		{
			xTimeToWait = comTX_MIN_BLOCK_TIME;		// 0x32
		}
...
portBASE_TYPE xResyncRequired = pdFALSE, xErrorOccurred = pdFALSE;
	...
	for( ;; )
	{
		...
		// ( cExpectedByte = 'A'; 			cExpectedByte <= 'X'; 			cExpectedByte++ )
		{
			/* Block on the queue that contains received bytes until a byte is
			available. */
			if( xSerialGetChar( xPort, &cByteRxed, comRX_BLOCK_TIME ) )		 // comRX_BLOCK_TIME = 0xffff
			{
